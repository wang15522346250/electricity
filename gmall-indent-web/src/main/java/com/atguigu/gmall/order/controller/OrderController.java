...
public class OrderController {
    ...
    public String orderSplit(HttpServletRequest request){
        ...
        List<OrderInfo> orderInfoList= orderInfoService.splitOrder(orderId,wareSkuMap);
        List<Map> list=new ArrayList<>();
        for (OrderInfo orderInfo : orderInfoList) {
            Map<String, Object> map = orderInfoService.getMap(orderId);
            list.add(map);
        }
        //把List转换为json串
        String jsonGwareList = JSON.toJSONString(list);
        return jsonGwareList;
